...
DatabaseLimiteduseStabilizedMyDomainHostnames `is set to`
`true` and myDomainSuffix is set to
...
Field in the object specified by the `inputObject` field in ObjectMapping on page 2216.
This field is mapped to the field in `outputField`, which is a field in the object specified
by the `outputObject` field in ObjectMapping on page 2216.
...
Field in the object specified by the `outputObject` field in ObjectMapping on page 2216.
This field is mapped to the field name in `inputField`, which is a field in the object
specified by the `inputObject` field in ObjectMapping on page 2216.
...
[deploying, and retrieving metadata, see Deploying and Retrieving Metadata with the Zip File. You can also deploy and retrieve the](https://developer.salesforce.com/docs/atlas.en-us.262.0.api_meta.meta/api_meta/file_based_zip_file.htm)
...
#### Metadata Types OmniChannelSettings OmniChannelSettings
...
`enableOmniSecondaryRoutingPriority` boolean Indicates whether Secondary Routing Priority is enabled in your org. The
default value is `false` . Available in API version 47.0 and later.

`enableOmniSkillsRouting` boolean Indicates whether skills-based routing is enabled in your org. The default
value is `false` .

`enableOmniStatusCapModel` boolean Indicates whether the status based capacity model for rep work is
...
**•** `ScratchOrgManagementPref on page 2060` (available
...
found within OrgSettings components. In API version 51.0 and later, those fields are found within MailMergeSettings on page 2193.
...
#### PartyDataModelSettingsPartyDataModel.settings file in the settings directory. The .settings files are
...
[management tasks require the Manage Encryption Keys permission. For a complete list of required permissions, read Which User](https://developer.salesforce.com/docs/atlas.en-us.262.0.securityImplGuide.meta/securityImplGuide/security_pe_permissions.htm#!)
[Permissions Does Shield Platform Encryption Require?.](https://developer.salesforce.com/docs/atlas.en-us.262.0.securityImplGuide.meta/securityImplGuide/security_pe_permissions.htm#!)
...
[segments. For more information, see Enhanced Usage Metrics in the](https://developer.salesforce.com/docs/atlas.en-us.262.0.platform_events.meta/platform_events/platform_events_monitor_enhanced_usage.htm) _Platform Events_
...
`arePredfndStatusValOveride` boolean Indicates whether predefined status validations are overridden ( `true` )
or not ( `false` ) when the Override Predefined Status Validations feature

is enabled. The default value is `false` . Available in API version 65.0
and later.
...
Foreign key to ObjectMapping on page 2259 that maps fields from the
...
on page 2258 advanced account forecast set.
...
on page 2260 SalesAgreementSettings.
...
The following is an example of a SchemaSettings
...
[components and apps. For more information, see Boxcar](https://developer.salesforce.com/docs/atlas.en-us.262.0.lightning.meta/lightning/controllers_server_actions_boxcar_dynamic.htm)
[Grouping and Optimization in the](https://developer.salesforce.com/docs/atlas.en-us.262.0.lightning.meta/lightning/controllers_server_actions_boxcar_dynamic.htm) _Lightning Aura Components_
_Developer Guide_ .
...
[attacks, use the CSPTrustedSite metadata type.](https://developer.salesforce.com/docs/atlas.en-us.262.0.api_meta.meta/api_meta/meta_csptrustedsite.htm)
...
restricted. If `false`, access isn’t restricted.
...
Reserved for future use. The default value is `allow` .

Available in API version 67.0 and later.

```
vfInlineScriptInjection
...
```
   ...
   <identityConfirmationOnTwoFactorRegistrationEnabled>true</identityConfirmationOnTwoFactorRegistrationEnabled>
       ...
       <referrerPolicyDirective>strict-origin-when-cross-origin</referrerPolicyDirective>
        ...
        <vfInlineScriptInjection>allow</vfInlineScriptInjection>
...
Profile names are also exposed when users with
permissions to perform the following tasks take these
actions:

**•** Create a tab or record type with a wizard step that
includes the assignment of tabs and record types to
profiles.

**•** Configure a login flow where viewing profile lists is
required to make flow associations.

**•** Set up delegated admins where looking up profiles
is needed to identify assignable profiles.

**•** Administer an org as a delegated customer admin.

**•** Administer an org as a delegated admin to view and
assign profiles of the delegated group.
...
### Metadata Types SharedTo SharedTo SharedTo defines the sharing access for a list view or a folder. It can be used to specify the target and source for owner-based sharing
...
### Note: SharedTo on page 2326 is included in the metadata for shared and private list views. SharedTo on page 2326 isn’t in the

metadata for public list views.

Declarative Metadata File Suffix and Directory Location

### SharedTo on page 2326 is used with ListView, Folder, and SharingRules.

Version

### SharedTo on page 2326 is available in API version 17.0 and later.
...
field.This field is available in API version 22.0 and later and is only
...
Use the roleAndSubordinates field instead for API version 22.0
and later.
...
Use the territoryAndSubordinates field instead for API version
22.0 and later.

A list of territories with sharing access. Use this field instead of
the territories

field. If you’re using Sales Territories, use
...
A list of territories with sharing access. All territories below each
of these territories in the territory hierarchy also have sharing
access. Use this field instead of the territoriesAndSubordinates
...
Note: You can’t create a SharingBaseRule on page 2328 component directly. Use the components under SharingRules instead.

Version

SharingBaseRule on page 2328 replaces BaseSharingRule and is available in API version 33.0 and later.
...
#### BaseSharingRule on page 2338 components are available in API version 24.0 and later.
...
UserMembershipSharingRule is used by the `ownerRules` field in UserSharingRules on page 2334.
...
**Description**
Indicates whether the unit of measure is used in the generated material waste
...
### Metadata Types SustnUomConversion SustnUomConversion
...
### Metadata Types SvcCatalogFulfillmentFlow SvcCatalogFulfillmentFlow
...
SvcCatalogFulfillFlowItem on page 2386[]
...
**Description**
Name of the related `SvcCatalogFulfillmentFlow` on page 2385, which
represents the flow associated with a specific catalog item in the Service Catalog.
Available in API version 56.0 and later.
...
2426 are specific to a single language, specify only that language. If the synonyms
...
Tag components have the suffix `.tag` and are stored in the `Tags` folder.
...
`name` string Required. The unique name of the tag. Can contain only underscores and
alphanumeric characters and must be unique in your org. It must begin with
...
### TagSet components have the suffix .tagSet and are stored in the tagSet folder.

Version

### TagSet components are available in API version 67.0 and later.

Fields

**Field Name** **Field Type** **Description**

`description` string A description of the tag set.

`label` string Required. Label that identifies the tag set in the Salesforce user interface.

`name` string Required. The unique name of the tag set. Can contain only underscores and
alphanumeric characters and must be unique in your org. It must begin with

a letter, not include spaces, not end with an underscore, and not contain two
consecutive underscores.

`options` TagOption[] A list of options associated with the tag set.

`properties` TagProperty[] A list of properties associated with the tag set.

`tags` Tag[] A list of tags included in this tag set.
...
### Metadata Types Territory Territory
...
on page 2411 `State equals California` .
...
**•** `ReportAnomalyEventStore`                            - This value is available
in API version 49.0 and later.
