...
[Required. The programmatic name of a custom lookup field on the LearningItem](https://developer.salesforce.com/docs/atlas.en-us.262.0.object_reference.meta/object_reference/sforce_api_objects_learningitem.htm)
[object that references the custom object used with this custom exercise.](https://developer.salesforce.com/docs/atlas.en-us.262.0.object_reference.meta/object_reference/sforce_api_objects_learningitem.htm)
...
[LearningItemType object.](https://developer.salesforce.com/docs/atlas.en-us.262.0.object_reference.meta/object_reference/sforce_api_objects_learningitemtype.htm)
...
`ai` base64Binary An internal AI description of the Lightning web component. This description
[is supported only in orgs that have Setup with Agentforce enabled. This](https://help.salesforce.com/s/articleView?id=xcloud.setup_agentforce_overview.htm&language=en_US)
...
[Component for Use in Setup with Agentforce.](https://developer.salesforce.com/docs/platform/lwc/guide/use-config-for-agentforce.html)
...
The following is an example of a LightningExperienceTheme component. See BrandingSet on page 541 for an example of the BrandingSet
component.
...
**•** [To pass the AppExchange Security Review, you must set the](https://developer.salesforce.com/docs/atlas.en-us.262.0.packagingGuide.meta/packagingGuide/security_review_guidelines.htm) `isExposed` field to `false` .
...
[Message Channel in Visualforce can’t pass the AppExchange Security Review. See Considerations and Limitations in the](https://developer.salesforce.com/docs/atlas.en-us.262.0.pages.meta/pages/message_channel_considerations.htm)
...
**–** `lightningMobileGenAi` (Agentforce Employee agent on Mobile and Agentforce Service agent via Enhanced Chat v2 on
Mobile)
...
### Metadata Types LiveChatAgentConfig LiveChatAgentConfig
...
### LiveChatButton on page 1527 configurations are stored in the <developer_name>.liveChatButton file in the

`liveChatButtons` directory.

Version

### LiveChatButton on page 1527 is available in API version 28.0 and later.
...
deployment. You must have the

CustomDomain permission enabled in your
organization before you can use a Salesforce
site with Chat.
...
LoyaltyProgramProcess[] on page 1537
...
LoyaltyProgramProcessParameter[] on page 1539
...
LoyaltyProgramProcessRule[] on page 1543
...
LoyaltyProgramProcessCondition on page 1541
...
LoyaltyProgramProcessConditionFilterCriteria[] on page 1541
...
LoyaltyProgramProcessAction[] on page 1544
...
LoyaltyProgramProcessRuleStepMapping[] on page 1549
...
LoyaltyProgramProcessActionParameter[] on page 1548
...
Note: When `nodeType on page 1553` is set to `NAMEFIELD` on
...
MarketingAppExtAction on page 1565[]
...
MarketingAppExtActivity on page 1562[]
...
**•** `SipVoice` —Represents an Agentforce Voice channel that uses SIP. Available
in API version 65.0 and later.
...
### MilestoneType on page 1588 is available in API version 27.0 and later.
...
See MobileSecurityAssignment on page 1601.
...
`disablePvtPagesForPwdResetInLWR` boolean Determines whether private pages in an LWR site are
available to users who haven’t completed setting up or

resetting their password. Set to `(true)` by default for
all LWR sites except those that use custom Visualforce
password-management pages.

When set to `(true)`, private pages are unavailable until
password setup or reset is complete. In LWR sites that use
custom Visualforce pages for password management,
make these pages public. Otherwise, visitors can’t access
them to set up or reset their password.
...
### Metadata Types NotificationTypeConfig NotificationTypeConfig
...
`appSettings` AppSettings on page 1659[] An array of settings for the connected apps supported for a notification type.
...
### Metadata Types OcrSampleDocument OcrSampleDocument
...
### Metadata Types OcrTemplate OcrTemplate
...
[To access this type, Omni-Channel must be enabled.](https://help.salesforce.com/articleView?id=omnichannel_intro.htm&language=en_US)
...
**Description**
Represents AI agent bots that are visible to supervisors in this configuration. This field
...
**Description**
Represents the supervisor to which the Omni-Channel supervisor configuration applies.
User-level configurations override profile-level configurations. This object is available
...
Represents AI agent bot definitions that are visible to supervisors in this configuration. This object is available in API version 64.0 and
later.
...
**Description**
Required. The name of the bot definition that is visible to supervisors in this
configuration.
...
**Description**
[The name of the FlexiPage added as a customized tab. Required when](https://developer.salesforce.com/docs/atlas.en-us.262.0.api_meta.meta/api_meta/meta_flexipage.htm) `tabType` is set to
`FlexipageType` .
...
Represents the supervisor to which the Omni-Channel supervisor configuration applies. User-level configurations override profile-level
configurations. This object is available in API version 64.0 and later.
...
**Description**
Required. Represents the supervisor to which the Omni-Channel supervisor configuration
applies.
...
### Metadata Types OrchestrationPlanCtxMapping
...
Represents the context mapping that Dynamic Revenue Orchestrator (DRO) uses to generate and orchestrate a plan for an object, such
...
### OrchestrationPlanCtxMapping components have the suffix .orchestrationPlanCtxMapping and are stored in the
...
[ContextDefinition that the orchestration use case is based on.](https://developer.salesforce.com/docs/atlas.en-us.262.0.api_meta.meta/api_meta/meta_contextdefinition.htm)

**Field Type**
string

**Description**
Name of the item-level node in the context definition. The item node represents the
line items or child records of the root node, such as the items in an order.
...
Name of the context mapping that maps the source object fields to the nodes in the
context definition.
...
Name of the root node in the context definition. The root node represents the parent
record that the orchestration plan is generated for, such as an order.
...
API name of the object that the orchestration plan is generated for. The combination
of `objectName` and `orchestrationType` must be unique.
...
on page 1686[] connection. Specify a name-value pair for each of the properties.
...
OnboardingDataObject[] on page 1690
...
OnbrdDataObjGrpDocCatgMap[] on page 1690
...
[DocumentCategory[]](https://developer.salesforce.com/docs/atlas.en-us.262.0.api_meta.meta/api_meta/meta_documentcategory.htm)
...
[DocumentType[]](https://developer.salesforce.com/docs/atlas.en-us.262.0.api_meta.meta/api_meta/meta_documenttype.htm)
...
OnboardingDataObjectField[] on page 1692
...
**Description**
[Reference to the PartyProfileDataObjectValidityDefinition metadata type](https://developer.salesforce.com/docs/atlas.en-us.262.0.api_meta.meta/api_meta/meta_partyprofiledataobjectvaliditydefinition.htm)
...
### Metadata Types PathAssistant PathAssistant
...
GatewayType Defines the payment gateway type.
(enumeration of
...
[For more information, see the Managed Component Access section of Sample package.xml Manifest Files in the](https://developer.salesforce.com/docs/atlas.en-us.262.0.api_meta.meta/api_meta/manifest_samples.htm) _Metadata API Developer_
_Guide_ .
...
`genComputingSummaryDefAccess` PermissionSetGenComputingSummaryDefAccess
on page 1708[]

Indicates the enhanced summary access configuration
for users assigned through a permission set. Available in
...
PermissionSetGenComputingSummaryDefAccess represents the enhanced summary access configuration for users assigned through
a permission set. Available in API version 66.0 and later.

**Field Name** **Field Type** **Description**

`configName` string Required. Indicates whether the users assigned to this profile can access
the enhanced summary configuration ( `true` ) or not ( `false` ).

`enabled` boolean Required. Indicates whether the access configuration is enabled ( `true` )
or not ( `false` ).

PermissionSetApplicationVisibility

PermissionSetApplicationVisibility on page 1708 determines whether an app is visible to a user assigned to this permission set.
...
PermissionSetApexClassAccess on page 1708 represents the Apex class access for users assigned to a permission set.
...
PermissionSetCustomMetadataTypeAccess on page 1709 represents the custom metadata type access for users assigned to a permission
...
PermissionSetExternalCredentialPrincipalAccess on page 1710 represents the access to the external credential’s principals. Users assigned
...
PermissionSetExternalDataSourceAccess on page 1710 represents the data source access for users with identity type of `Per User` .
...
PermissionSetFieldPermissions on page 1710 represents the field permissions for users assigned to a permission set. In API version 30.0
...
PermissionSetFlowAccess on page 1711 represents which flows a permission set grants access to. Available in API version 47.0 and later.
...
by the object field can be viewed, edited, or deleted by the users

assigned to this permission set ( `true` ) or not ( `false` ), regardless
...
of the sharing settings for the object. Includes private records
(records with no parent object). Similar to the Modify All Data user
...
PermissionSetApexPageAccess on page 1712 represents the Visualforce page access for users assigned to a permission set.
...
PermissionSetRecordTypeVisibility on page 1712 represents the visibility of record types for this permission set.
...
PermissionSetTabSetting on page 1712 represents the tab settings for a permission set.
...
[For more information, see User License Restriction Categories](https://developer.salesforce.com/docs/atlas.en-us.262.0.plp_dev.meta/plp_dev/partner_licensing_platform_restriction_categories.htm)
[(Developer Preview). This field is available in API version 55.0](https://developer.salesforce.com/docs/atlas.en-us.262.0.plp_dev.meta/plp_dev/partner_licensing_platform_restriction_categories.htm)
and later.
...
### Metadata Types PersonAccountOwnerPowerUser PersonAccountOwnerPowerUser
...
[For an example of a custom channel that holds custom platform events and Real-Time Event Monitoring events, see Group Platform](https://developer.salesforce.com/docs/atlas.en-us.262.0.platform_events.meta/platform_events/platform_events_subscribe_custom_channels.htm)
[Events into One Stream with a Custom Channel in the](https://developer.salesforce.com/docs/atlas.en-us.262.0.platform_events.meta/platform_events/platform_events_subscribe_custom_channels.htm) _Platform Events Developer Guide_ .

SEE ALSO:

_[Change Data Capture Developer Guide](https://developer.salesforce.com/docs/atlas.en-us.262.0.change_data_capture.meta/change_data_capture/cdc_subscribe_channels.htm)_ : Subscription Channels
...
[even when not changed. For more information, see Enrich Change Events](https://developer.salesforce.com/docs/atlas.en-us.262.0.change_data_capture.meta/change_data_capture/cdc_enrich_intro.htm)
[with Extra Fields When Subscribed with CometD in the](https://developer.salesforce.com/docs/atlas.en-us.262.0.change_data_capture.meta/change_data_capture/cdc_enrich_intro.htm) _Change Data_
...
[For more information, see Filter Your Stream of Platform Events with](https://developer.salesforce.com/docs/atlas.en-us.262.0.platform_events.meta/platform_events/platform_events_filter_section.htm)
...
[Stream of Change Events with Channels in the](https://developer.salesforce.com/docs/atlas.en-us.262.0.platform_events.meta/platform_events/cdc_filter_section.htm) _Change Data Capture_
...
### Metadata Types PlatformEventMigration
...
_Change Data Capture Developer Guide_ [: Example Diagrams for Channels and Channel Members](https://developer.salesforce.com/docs/atlas.en-us.262.0.change_data_capture.meta/change_data_capture/cdc_channel_examples.htm)

_Change Data Capture Developer Guide_ [: Filter Your Stream of Change Events with Channels](https://developer.salesforce.com/docs/atlas.en-us.262.0.change_data_capture.meta/change_data_capture/cdc_filter_section.htm)

_Platform Events Developer Guide_ [: Filter Your Stream of Platform Events with Channels](https://developer.salesforce.com/docs/atlas.en-us.262.0.platform_events.meta/platform_events/platform_events_filter_section.htm)
...
**Description**
Required. The API name of the standard volume platform event that needs to be
migrated to a high volume platform event.
...
The following is an example of a PlatformEventMigration component.
...
```
        ...
        <Package xmlns="http://soap.sforce.com/2006/04/metadata">
         ...
         <types>
           ...
           <members>MyEventMigration</members>

           <name>PlatformEventMigration</name>
...
**2.** Next, create a .zip file containing the configuration data. Create a working directory on your desktop: ~/Desktop/MyMigrationConfig
Inside this directory, create another directory ~/Desktop/MyMigrationConfig/platformEventMigrations. Inside of that directory, create
the file ~/Desktop/MyMigrationConfig/platformEventMigrations/MyEventMigration.platformEventMigration Next and paste this
XML as the body of that file and modify the eventObject element to match your standard volume platform event’s API name.
...
masterLabel can be any string. eventObject must be the API name of the standard volume platform event that you want to migrate
to a high volume platform event.
...
members should contain the name of the previously created file without the .platformEventMigration extension. version must be
67.0 or higher.

**4.** Create a .zip file out of your configuration files. On Windows:

**a.** Select both the platformEventMigrations folder and package.xml file

**b.** Right-click  Send to  Compressed (zipped) folder

**c.** Name it migration-config.zip
...
**c.** Create a clean ZIP without hidden files

```
       zip -r migration-config.zip MyMigrationConfig -x "*.DS_Store" -x "__MACOSX"
...
**b.** Navigate to the MyMigrationConfig folder

```
       cd ~/Desktop/MyMigrationConfig

```

**c.** Create the .zip file

```
       zip -r migration-config.zip platformEventMigrations package.xml
...
```
     sf org login web --instance-url $URL --alias my-org
...
```
     sf project deploy start --metadata-dir migration-config.zip --target-org my-org
...
**8.** To check the status of the migration, in Salesforce click the gear icon  Developer Console. Click "Query Editor" tab. Check "Use Tooling
API" checkbox. Run this query after replacing the value for DeveloperName with the value you used:

```
            SELECT Id, DeveloperName, MasterLabel, EventObject, MigrationStatus,

     StartDate, EndDate, HighVolumePosition

            FROM PlatformEventMigration

            WHERE DeveloperName = 'MyEventMigration'

```

You should see your configuration record with: Your MasterLabel, EventObject = Your Platform Event, MigrationStatus = 'MIGRATING'
(initially). MigrationStatus will change to COMPLETED when the migration is complete. You’ll also receive an email when the migration
is complete. While the migration is running, publishes won’t work and you’ll be unable to create new triggers or flows until the
migration finishes.
...
[from 1 through 10. See Platform Event Processing at Scale with Parallel](https://developer.salesforce.com/docs/atlas.en-us.262.0.platform_events.meta/platform_events/platform_events_ps.htm)
[Subscriptions for Apex Triggers in the](https://developer.salesforce.com/docs/atlas.en-us.262.0.platform_events.meta/platform_events/platform_events_ps.htm) _Platform Events Developer Guide_ .
...
[key for parallel subscriptions. See Platform Event Processing at Scale with](https://developer.salesforce.com/docs/atlas.en-us.262.0.platform_events.meta/platform_events/platform_events_ps.htm)
[Parallel Subscriptions for Apex Triggers in the](https://developer.salesforce.com/docs/atlas.en-us.262.0.platform_events.meta/platform_events/platform_events_ps.htm) _Platform Events Developer_
_Guide_ .
...
**Description**
Required. Foreign key to the permissionSet on page 1704 entity.
...
**Description**
Required. Foreign key to the profile on page 1757 entity.
...
### Metadata Types PostTemplate PostTemplate
...
### productAttributeSetItems ProductAttributeSetItem A list of ProductAttributeSetItem. PresenceDeclineReason
